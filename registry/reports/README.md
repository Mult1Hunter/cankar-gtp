...
- **snapshot** (corpus-quality.md, near-duplicates.md, dlib-reconcile.md,
  merge.md): computed from gitignored data/ or live dLib state by
  `cankar corpus stats` / `dedup` / `reconcile-dlib` / `merge`; CI cannot
  regenerate them - do NOT fold them into `report --all` or the drift gate
  goes red.
